...
import { z } from 'zod';
import { readSettings, writeSettings, type DefaultProviderChoice } from './settings';
import {
  ChatModeSchema,
  EffortLevelSchema,
...
import {
  FILE_LIST_DEFAULT_MAX_FILES,
  FILE_LIST_HARD_MAX_FILES,
  enumerateWorkspaceFiles,
  inspectWorkspaceContext,
  runSafeWorkspaceCommand,
} from './workspace/repoContext';
import { registerAppPreferenceHandlers } from './ipc/appPreferenceHandlers';
...
function registerWorkspaceHandlers(electronApp: App): void {
  ...
  ipcMain.handle(
    ...
    async (_evt, raw: unknown): Promise<Result<FileEntry[]>> => {
      try {
        ...
        const { files } = await enumerateWorkspaceFiles(root, args.ignore_patterns ?? [], cap);
        return ok(files);
      } catch (err) {
        return fail(err);
      }
  ...
  // ── workspace/inspect (local repo coding loop v1) ───────────────────
  // RepoIndex-lite: gitignore-aware file scan, package scripts, safe test
  // commands, git status, and context candidates. This is intentionally
  // read-only so opening a repo never mutates the workspace.
  ipcMain.handle(
    'workspace/inspect',
    async (_evt, raw: unknown): Promise<Result<RepoContextSummary>> => {
      try {
        return ok(await inspectWorkspaceContext(raw));
      } catch (err) {
        return fail(err);
      }
    }
  ...
  // ── workspace/run-safe-command (local repo coding loop v1) ───────────
  // Explicit user-triggered, allowlisted validation commands only. No shell,
  // no arbitrary command string, and no push/delete/build side effects.
  ipcMain.handle(
    'workspace/run-safe-command',
    async (_evt, raw: unknown): Promise<Result<CommandRunResult>> => {
      try {
        return ok(await runSafeWorkspaceCommand(raw));
      } catch (err) {
        return fail(err);
      }
    }
