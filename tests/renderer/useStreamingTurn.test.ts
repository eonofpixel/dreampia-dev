...
describe('useStreamingTurn', () => {
  ...
  it('tool_calls appear after tool_call events', async () => {
    const provider = new MockProvider({
      responseText: 'calling tool', delayMs: 0,
  ...
  it('handles error event by calling onError', async () => {
    const errorProvider = {
      ...
      async *stream() { yield { type: 'error' as const, error: 'test error' }; },
    };
    const onError = vi.fn();
    const { result } = renderHook(() =>
      useStreamingTurn({ provider: errorProvider, onTurnUpdate: vi.fn(), onComplete: vi.fn(), onError })
    );
    await act(async () => { await result.current.start(input); });
